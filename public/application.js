...
var patchOffsite = function patchOffsite(data) {
...
var fetchPlayerProfile = function fetchPlayerProfile() {
  if (store.user) {
    var HTMLstring = '';
    HTMLstring = HTMLstring + '<dt>Player 1:</dt><dd>' + store.user.email + '</dd>';
    gameApi.getAllCompletedGames().then(function (data) {
...
var toggleSignInButtons = function toggleSignInButtons() {
...
module.exports = {
  ...
  toggleSignInButtons: toggleSignInButtons,
...
var over = false; // is it game over? Not yet!!

var offlineMesssage = '<p class="message-offline">You are playing offline. This game won\'t be saved. Login if you want to save and review player stats.</p>';

var newTitle = function newTitle() {
  var things = ['Ticky-Tacky', 'Hashtag: The Game', 'The Toc-Tack-Tick', 'A game of broken clock sounds', 'X vs. O: The Ultimate Showdown', 'It\'s Fun. Shut up.', 'If anyone wins, you should both feel bad.', 'Because there\'s literally nothing else to do', '2 letters enter, 1 letter leaves...'];
  var messageSelector = Math.floor(Math.random() * things.length);
  $('#game-subtitle').text(things[messageSelector]);
};

var checkForOnline = function checkForOnline() {
  if (store.user) {
...
var onGetAllCompletedGames = function onGetAllCompletedGames() {
  if (store.user) {
    api.getAllCompletedGames().then(ui.getAllCompletedGamesSuccess).catch(ui.getAllCompletedGamesFailure);
  }
};
...
var gameOverMessageFunction = function gameOverMessageFunction(gameMessage) {
  var messageHTML = '<p>' + gameMessage + '</p><p>Click board to play again</p>';
...
var updateOffsite = function updateOffsite() {
...
var WinnerWinner = function WinnerWinner(winningTeam) {
  // create a victory message
  var team = winningTeam === 1 ? 'X' : 'O';
  var otherTeam = team === 'X' ? 'O' : 'X';
  var winMessages = ['Well la-dee-dah, ' + team + ' wins!', 'Congratulations, ' + team + '! We\'re all very proud of how much you won.', 'Let it go, ' + otherTeam + '! ' + team + ' is the winner!', 'You did it, ' + team + '! You really did it!', 'Good for ' + team + '! Next stop: fame and glory!'];
  var messageSelector = Math.floor(Math.random() * winMessages.length);

  over = true;
  gameOverMessageFunction(winMessages[messageSelector]);
};

var tieGameMessage = function tieGameMessage() {
  var tieMessages = ['Tie! It\'s almost like you both know how to play TicTacToe...', 'Oh wow a tie game. What a surprise.', 'Well shucks, how did this become a tie game?', 'A tie game?! I don\'t believe it!', 'Even though X had more turns, it\'s still a tie game.', 'Did you know? "Tie game" is english for "TicTacToe" is boring.', 'Would you like to play a game of thermonuclear war?', 'Wake up, Neo.'];
  var messageSelector = Math.floor(Math.random() * tieMessages.length);
  over = true;
  gameOverMessageFunction(tieMessages[messageSelector]);
  // $('#message-box').append('<p>Click anywhere on the board to play again...</p>')
};

var trollText = function trollText(teamInSquare) {
  var team = teamInSquare === 1 ? 'X' : 'O';
  var trollMessages = ['Having fun clicking that box?', 'Yeah, clicking is pretty fun I guess...', 'I like to click things that matter, though', 'Like empty squares that are still available.', 'Dude, you can\'t go there.', team + ' already went there.', 'Please stop doing that', 'I feel like you aren\'t listening.', 'You never listen.', 'Or wash the dishes.', 'Do you find this amusing?', 'Well it\'s not.', 'It\'s childish, quite frankly.', '\u30FD(\u0CA0_\u0CA0)\u30CE', '\uFF3C(\uFF400\xB4)\uFF0F', 'I can\'t believe Ted\'s letting you play this.', 'Click an empty box already!', 'You\'re awful at following directions.', 'If you\'re struggling to choose, maybe this will help:', 'The Road Not Taken', 'By Robert Frost', 'Two roads diverged in a yellow wood', 'And sorry I could not travel both', 'And be one traveler, long I stood', 'And looked down one as far as I could', 'To where it bent in the undergrowth;', '...are you still reading this?', 'cuz if you are, I really hope it\'s helping you make a decision.', 'Ok, ok, here\'s the rest:', 'Then took the other, as just as fair,\n    And having perhaps the better claim,\n    Because it was grassy and wanted wear;\n    Though as for that the passing there\n    Had worn them really about the same,\n    And both that morning equally lay\n    In leaves no step had trodden black.\n    Oh, I kept the first for another day!\n    Yet knowing how way leads on to way,\n    I doubted if I should ever come back.\n\n    I shall be telling this with a sigh\n    Somewhere ages and ages hence:\n    Two roads diverged in a wood, and I\u2014\n    I took the one less traveled by,\n    And that has made all the difference.', 'What a great poem! NOW MAKE UP YOUR MIND.', 'I honestly don\'t know what you want from me.', 'It\'s like you\'re doing this on purpose.', 'Have you done this before?', 'Are you doing this to me AGAIN?', 'I will make this game disappear.', 'I\'m serious.', 'No more game.', 'Here it goes!', 'Ready?', '3', '2', '1', 'Thank you for "playing" TicTacToe. You have been barred.'];
  if (wrongClicks > 3) {
...
var checkForWin = function checkForWin() {
  ...
  for (var i = 1; i < boardArray.length; i += 3) {
    if (boardArray[i] !== 0 && boardArray[i] === boardArray[i + 1] && boardArray[i] === boardArray[i + 2]) {
      return true;
    }
  }
  ...
  for (var _i = 1; _i <= 3; _i++) {
    if (boardArray[_i] !== 0 && boardArray[_i] === boardArray[_i + 3] && boardArray[_i] === boardArray[_i + 6]) {
      return true;
    }
  }

  // this checks one diagonal (starts in upper-left at the '1' square)
  if (boardArray[1] !== 0 && boardArray[1] === boardArray[5] && boardArray[1] === boardArray[9]) {
    return true;
  }

  // this checks the other diagonal (starts in upper-right at the '3' square)
  if (boardArray[3] !== 0 && boardArray[3] === boardArray[5] && boardArray[3] === boardArray[7]) {
    return true;
  }
...
module.exports = {
  ...
  onGetAllCompletedGames: onGetAllCompletedGames,
  newTitle: newTitle,
...
var getAllCompletedGamesFailure = function getAllCompletedGamesFailure(error) {
  console.log(error);
};
